...
ProductRow: TypeAlias = tuple[int, str, str, str, str, float]
ProductCatalog: TypeAlias = list[ProductRow]


def create_random_produts(n: int = 100, rng: random.Random = None) -> ProductCatalog:
...
def print_product_catalog(content: ProductCatalog) -> None:
    df = pd.DataFrame(content, columns=["product_id", "name", "brand", "category", "color", "base_price"])
    print_html(df.sample(n=10), title="Sample Product Catalog")
...
def create_transaction_db(
    ...
    product_catalog = create_random_produts(n=n_products, rng=rng)
    print(f"Generated product catalog with {n_products} products.")
    ...
    for (pid, name, brand, category, color, base_price) in product_catalog:
        ...
        """, (pid, name, brand, category, color, initial_stock, base_price, 
              f"Initial stock of {initial_stock} units at ${base_price} each."))
        
        current_price = base_price

        # follow-up events (restocks, sales, price updates)
        for _ in range(n_txns_per_product - 1):
            ...
            if event_type == 'restock':
                ...
                """, (pid, name, brand, category, color, qty, current_price,
                      f"Sold {-qty} units at ${current_price} each."))
            
            else:  # price_update
                price_change = round(rng.uniform(-5.0, 5.0), 2)
                new_price = max(1.0, round(current_price + price_change, 2))  # Ensure price doesn't go below $1
                cursor.execute("""
                    INSERT INTO transactions (
                               product_id, product_name, brand, category, color, 
                               action, qty_delta, unit_price, notes
                    ) VALUES (?, ?, ?, ?, ?, 'price_update', 0, ?, ?)
                """, (pid, name, brand, category, color,
                      new_price,
                      f"Price updated to ${new_price} from ${current_price} ."))
